...
my  $rs_perform;   # refernce to $Perform::__perform, TRUE to perform/execute step or 
   ...
   eval {
     ...
     for my $j ($pos1..$#steps) {
       ...
       if ( ( not (defined($steps[$j][0])) )  or ($steps[$j][0] eq 'ALWAYS')) {
         $obj = $steps[$j][1]();
         ouch 'No object returned', 'No object returned' if (not $obj);
         $touched[$pos2]= GiveMeTheBehavior($steps[$j][0]) . $obj->arg_text;
     ...
     if (not ($obj->rc)) {
       ...
       for my $j ($pos1..$#steps) {
         ...
         if ((defined($steps[$j][0])) and 
           ...
           $object = $steps[$j][1]();
           ouch 'No object returned', 'No object returned' if (not $object);
           $touched[$j]= GiveMeTheBehavior($steps[$j][0]) . $object->arg_text;
           # what do you do with a bad return code??
           ouch 'Bad, Cleanup', 'Bad, Cleanup' if (not ($object->rc));
         }
       }
     }
     if ($TESTING) {
       ...
       while (my ($j,$step) = each @steps) {
         ...
         $object = $step->[1]();
         ouch 'No object returned', 'No object returned' if (not $object);
         $touched[$j]= GiveMeTheBehavior($step->[0]) . $object->arg_text;
       }
       $$rs_perform=1;
     }
     end_trans();
